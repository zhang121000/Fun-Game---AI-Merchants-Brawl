...
```
backend/                          frontend/
├── main.py        FastAPI 入口    ├── src/App.tsx     路由
├── config.py      pydantic 配置   ├── src/pages/      7 个页面
├── constants.py   共享常量        ├── src/api/        8 个 API 模块
├── core/database.py SQLAlchemy   ├── src/stores/     Zustand 状态
├── models/        7 个 ORM 模型   ├── src/components/ 2 个组件
├── api/           8 个路由        └── vite.config.ts  Vite 代理
├── services/      业务服务
├── ai/            AI Provider 体系
...
**前端全局错误处理**（`frontend/src/api/client.ts`）：axios 响应拦截器统一捕获超时、网络错误、500/404/400 等，通过 antd `message.error()` 提示，页面不再卡 loading。

**状态管理**：`customerStore` 管理当前模拟顾客身份切换，`cartStore` 管理购物车增删改查。未引入全局状态管理。
